...
import pandas as pd
import numpy as np
from lazyopt import LazyOptimizer
...
def load_fasttext(word_index):
    ...
    EMBEDDING_FILE = 'embed_tweets_de_300D_fasttext'
    def get_coefs(word,*arr): return word, np.asarray(arr, dtype='float32')
...
#     glove,fasttext
    ...
    # sent = Embedding(input_dim=10000, output_dim=300, trainable=trainable,
    #                  name="sent")(sequence_input)
    # output1 = SparseSelfAttention(10,30)(sent)
    # output2 = SparseSelfAttention(10,30)(sent)
    # output = Concatenate()([output1, output2])
    output = KMaxPooling(3)(sent)
...
train_file = "data/clean_hasoc_2020_en_train.csv"
test_file = "data/clean_english_test_1509.csv"
...
clr = CyclicLR(base_lr=0.001, max_lr=0.006,
               ...
               gamma=0.99994)
filepath = "emd/h5/modelask1_{epoch:03d}-{val_loss:.4f}.h5" #避免文件名称重复
...
for k in range(NUM_SPLIT):
    ...
    trainb = data_generator(x_train, yb_train,batch_size=batch_size)
    valb = data_generator(x_val, yb_val,batch_size=batch_size)
...
# traina = data_generator(train_x, train_ya)
# vala = data_generator(val_x, val_ya)
# modela = SSAmodel(embedding_matrix,
#                       num_words,
#                       max_sequence_length,
#                       num_class=2,
#                       trainable=True)
# modela.fit_generator(traina.__iter__(),
...
#                      callbacks=callbacks,
#                      verbose=1)
# testa = modela.predict(test_x)
# #
# trainb = data_generator(train_x, train_yb,batch_size=batch_size)
# valb = data_generator(val_x, val_yb,batch_size=batch_size)
# # modelb = SSAmodel(embedding_matrix,num_words,max_sequence_length, num_class=4, trainable=True)
# modelb = SSAmodel(num_words,max_sequence_length,num_class=4, trainable=True)
# modelb.fit_generator(trainb.__iter__(),
#                      steps_per_epoch=len(trainb),
#
#                      epochs=epochs,
#                      validation_data=valb.__iter__(),
#                      validation_steps=len(valb),
...
save_result(ID,tweet_id,test_a,task='task1',file_name='submission_en_task1.csv')
...
save_result(ID, tweet_id, test_b, task='task2', file_name='submission_en_task2.csv')
